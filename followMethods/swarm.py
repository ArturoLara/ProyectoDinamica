...
class Particle:
    ...
    def move(self, knowledge_weight, swarm_vx=0, swarm_vy=0, use_vel=True, use_accel=False):
        dt = 1
        noise_pos = 2  # Aumentar ruido posicional
        noise_vel = 1  # Aumentar ruido en velocidad

        if use_vel:
            self.x += knowledge_weight * swarm_vx * dt +  self.vx * dt + np.random.normal(0, noise_pos)
            self.y += knowledge_weight * swarm_vy * dt + np.random.normal(0, noise_pos)
...
class Swarm:
    def __init__(self, roi_size, img, n_particles=100, video=None,
                 use_vel=True, use_accel=False, resample="pr", knowledge_weight=0.2):
        self.video = video
    ...
    def get_smoothed_estimate(self):
        ...
        self.swarm_vx = sum(p.vx * w for p, w in zip(self.particles, self.weights))
        self.swarm_vy = sum(p.vx * w for p, w in zip(self.particles, self.weights))
        return smoothed
    ...
    def run(self, steps=1):
        for _ in range(steps):
            ...
            for p in self.particles:
                p.move(self.knowledge_weight, self.swarm_vx, self.swarm_vy, self.use_vel, self.use_accel)
